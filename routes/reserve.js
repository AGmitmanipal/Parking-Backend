...
// This file used to run a second Express server (port 7000).
// It is now a router mounted by the main backend server so we only run ONE server.
...
router.delete("/reserve/:id", cancelReservationHandler);
// Legacy alias to support old frontend calling /reserve/del/:id
router.delete("/reserve/del/:id", cancelReservationHandler);
...
router.get("/zones/:zoneId/slots-status", async (req, res) => {
